  ...
  for (const configSource of ['default', 'custom'] as const) {
    ...
    const provider = new NgrokProvider({ authSource: 'system-config', configSource, configPath: 'C:\\fixture\\ngrok.yml', hostname: 'https://stable.ngrok.app' }, ingress,
      () => 's'.repeat(43), () => undefined, 'probe', () => undefined, {
        command: async (_exe, args) => { commands.push(args); return args[0] === 'version' ? 'ngrok 3.x' : 'Valid configuration' },
        spawn: (_exe, args, options) => {
          ...
          return child
        }, probe: async () => ({ modern: true, toolCount: 11, workspaceList: true }),
      })
    try {
      ...
      child!.emit('exit', 1)
      expect(provider.getStatus().errorCode).toBe('NGROK_ENDPOINT_ALREADY_ONLINE')
