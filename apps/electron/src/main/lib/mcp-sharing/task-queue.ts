...
export interface AnalysisJobRunner {
  run(workspaceId: string, instruction: string, signal: AbortSignal, bindSession: (id: string) => void, targetId?: string, audit?: (attempt: McpAgentAttempt) => void): Promise<string>
}
...
export class AnalysisTaskQueue {
  ...
  snapshot(): McpRemoteTask[] { return [...this.tasks.values()].map((t) => ({ ...t, attempts: t.attempts?.map((attempt) => ({ ...attempt })), changed_files: [...t.changed_files], warnings: [...t.warnings] })) }
  start(workspaceId: string, instruction: string, targetId?: string): McpRemoteTask {
    this.authorize(workspaceId)
    if (!instruction.trim() || instruction.length > 12000) throw new Error('TASK_INSTRUCTION_INVALID')
    const signature = createHash('sha256').update(workspaceId + '\0' + (targetId ?? '') + '\0' + instruction.trim()).digest('hex')
    ...
    if (this.starts.length >= 6 || [...this.tasks.values()].filter((t) => t.status === 'queued').length >= limits.maxQueued + Math.max(0, limits.maxConcurrent - this.active.size)) throw new Error('TASK_RATE_LIMIT')
    const task: McpRemoteTask = { id: 'pt_' + randomBytes(16).toString('hex'), workspaceId, targetId, attempts: [], createdAt: now, updatedAt: now, status: 'queued', changed_files: [], warnings: [] }
  ...
  get(id: string): McpRemoteTask {
    ...
    return { ...task, attempts: task.attempts?.map((attempt) => ({ ...attempt })), changed_files: [...task.changed_files], warnings: [...task.warnings] }
  }
  ...
  private async drain(): Promise<void> {
    ...
    try {
      ...
      const summary = await this.runner.run(task.workspaceId, this.pending.get(task.id)!.instruction, abort.signal, (id) => { task.sessionId = id; this.emit() }, task.targetId, (attempt) => { (task.attempts ??= []).push(attempt); this.emit() })
      ...
      clearTimeout(deadline); this.pending.delete(task.id); this.active.delete(task.id); task.updatedAt = Date.now(); this.emit(); void this.drain()
    }
  }
}
