...
import { ProviderBinarySetup } from './ProviderBinarySetup'
...
export function McpSharingSettings(): React.ReactElement {
  ...
  React.useEffect(() => {
    if ((window.electronAPI?.bridgeVersion ?? 0) < 13) return
  ...
  if (typeof window === 'undefined' || (window.electronAPI?.bridgeVersion ?? 0) < 13) return <SettingsSection title="ChatGPT / MCP 共享" description="后台版本过旧，请完全退出并重启 Proma。"><p>需要 bridge 13 或更高版本。</p></SettingsSection>
  ...
  const saveSharing = async () => {
    ...
    message('共享内容与能力已保存；内容、权限和 Agent 模式变化不会重连网络，工具定义变化后请在 ChatGPT Refresh。')
  }
  ...
  const setProviderSettings = (value: typeof selected) => { if (remote.provider) patchRemote({ providers: { ...remote.providers, [remote.provider]: { ...(remote.provider === 'ngrok' ? { mode: selected.mode ?? (selected.authSource === 'system-config' ? 'system' : 'proma-managed') } : {}), ...selected, ...value } } }) }
  ...
  return <SettingsSection title="ChatGPT / MCP 共享" description="先选择共享内容，再决定直接读取还是交给 PROMA 分析，最后连接 ChatGPT。">
    <div className="space-y-4">
      <SettingsCard divided={false}><div className="p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">已启用 {sharing.roots.filter((root) => root.enabled).length} 个共享项目。新增、移除和修改权限不会改变 ChatGPT 地址，也不会重启 ngrok。</p>
      ...
      <SettingsCard divided={false}><div className="p-4 space-y-3">
        ...
        <McpAgentTargetsCard value={sharing.delegation} channels={view.channels} onChange={(delegation) => patchSharing({ delegation, tools: { ...sharing.tools, fileWrite: sharing.tools.fileWrite || delegation.action?.write === true, shell: sharing.tools.shell || delegation.action?.execute === true } })} />
      ...
      <SettingsCard divided={false}><div className="p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs"><label className="flex gap-2"><input type="checkbox" checked={sharing.localEndpoint.enabled} onChange={(e) => patchSharing({ localEndpoint: { ...sharing.localEndpoint, enabled: e.target.checked } })} />本机 MCP 服务</label><SettingHelp topic="local-mcp-service" /><span className="text-muted-foreground">供本机客户端使用</span><Button size="sm" variant="outline" disabled={!view.sharingDirty || view.busy.sharing} onClick={() => { void run('sharing', saveSharing) }}>保存本机服务设置</Button></div>
        ...
        {provider && <>
          {provider.kind === 'ngrok' && <McpNgrokSettings status={live?.kind === 'ngrok' ? live : undefined} value={selected} onChange={setProviderSettings} run={run} />}
          <Button variant="link" onClick={() => { void window.electronAPI.openExternal(provider.priceUrl) }}>查看官方价格与限制</Button>
          {provider.kind !== 'ngrok' && <ol className="space-y-2">{provider.steps.map((step, index) => <li key={step.id} className="rounded border border-border/60 p-2 text-xs"><strong>{index + 1}. {step.title}</strong><p className="my-1 text-muted-foreground">{step.description}</p>{step.action?.kind === 'open-url' && step.action.url && <Button size="sm" variant="outline" onClick={() => { void window.electronAPI.openExternal(step.action!.url!) }}>打开官方页面</Button>}<span role="status">{view.guide[step.id]}</span></li>)}</ol>}
          {provider.controls.binary && !(provider.kind === 'ngrok' && selected.mode === 'external-existing') && <ProviderBinarySetup provider={provider} value={selected} onChange={setProviderSettings} busy={Boolean(view.busy.detect)} run={run} detect={() => window.electronAPI.detectMcpProvider({ provider: provider.kind, executablePath: selected.executablePath })} />}
          {(provider.controls.hostname === 'fixed') && <label className="block text-xs">{provider.kind === 'ngrok' ? 'Assigned Development Domain（HTTPS）' : '固定 HTTPS 地址'}<Input value={selected.hostname ?? ''} placeholder="https://mcp.example.com" onChange={(e) => setProviderSettings({ hostname: e.target.value })} /></label>}
      ...
      <SettingsCard divided={false}><div className="p-4 space-y-2"><h3 className="text-sm font-semibold">⑤ 状态与诊断</h3><p role="status" className="break-all text-xs">{view.message}</p><p className="text-sm">连接：{live?.phase ?? 'stopped'} · {live?.probe ? `${live.probe.toolCount} 个工具` : '尚未检查'}</p>{live?.discoveryFingerprint && <p className="break-all text-xs text-muted-foreground">Discovery：{live.confirmedDiscoveryFingerprint === live.discoveryFingerprint ? '已确认，无需 Refresh' : '能力说明或工具定义待 Refresh'}</p>}{live?.errorCode && <p role="alert" className="text-xs text-destructive">{live.errorCode} · {live.errorMessage}</p>}
        {live?.kind === 'ngrok' && live.errorCode && <div className="space-y-2 rounded border border-border p-3 text-xs"><p>{live.errorCode === 'NGROK_ENDPOINT_CONFLICT' ? '这个 ngrok 地址当前由另一个服务使用。请选择 PROMA 专用 Domain，或由你停止原服务；若现有 ngrok 本来就转发到 PROMA，选择“使用已经运行的 ngrok”。不会自动 pooling 或停止外部进程。' : live.errorCode === 'NGROK_DOMAIN_CONFIRMATION_REQUIRED' ? '请确认当前 Domain 是否为 PROMA 专用。' : '检查程序、配置、Credential 与 Domain。若账号没有另一个可独占 Endpoint，可使用其它 Credential / Account、调整账号 Endpoint，或切换连接方式。'}</p><div className="flex flex-wrap gap-2">{REMOTE_PROVIDERS.filter((p) => ['cloudflare-quick', 'cloudflare-named', 'tailscale-funnel', 'external-https'].includes(p.kind)).map((p) => <Button key={p.kind} size="sm" variant="outline" onClick={() => { setIntent(p.use); patchRemote({ provider: p.kind }) }}>切换到 {p.name}</Button>)}</div></div>}
        ...
        {view.schemaChanged || !view.schemaBaseline ? <p className="text-xs">{view.schemaChanged ? 'PROMA 能力说明或工具定义已变化，请在 ChatGPT 点击 Refresh。' : '尚未确认 ChatGPT 能力扫描状态。'}<Button variant="link" size="sm" onClick={() => { void run('schema', async () => { await window.electronAPI.confirmMcpToolSchema() }) }}>我已 Scan Tools / Refresh</Button></p> : <p className="text-xs text-muted-foreground">能力说明和工具定义未变化，无需 Refresh。</p>}
        <Button variant="outline" size="sm" disabled={view.busy.diagnose} onClick={() => { void run('diagnose', async () => { const result = await window.electronAPI.diagnoseMcpTransport(); message(result.checks.map((c) => `${c.level === 'warn' ? '⚠' : c.ok ? '✓' : '✕'} ${c.name}：${c.detail} ${c.ok ? '' : c.nextAction ?? ''}`).join('；')) }) }}>运行完整连接自检</Button>
        {view.tasks.length > 0 && <details><summary className="text-xs">PROMA Agent 任务（不导出指令）</summary>{view.tasks.toReversed().map((t) => <div key={t.id} className="mt-2 rounded border p-2 text-xs"><div className="flex flex-wrap items-center gap-2"><span>{t.id} · {t.kind === 'action' ? '动作' : '分析'} · {t.status}</span>{['queued', 'planning', 'running', 'waiting_approval'].includes(t.status) && <Button size="sm" variant="ghost" onClick={() => { void run('task', async () => { window.electronAPI.cancelMcpTask(t.id) }) }}>取消</Button>}{t.kind === 'action' && t.status === 'waiting_approval' && <><Button size="sm" onClick={() => { void run('approve-task', async () => { await window.electronAPI.approveMcpTask(t.id) }) }}>批准本次</Button><Button size="sm" variant="destructive" onClick={() => { void run('deny-task', async () => { await window.electronAPI.denyMcpTask(t.id) }) }}>拒绝</Button></>}</div><p>{t.summary?.slice(0, 500) ?? t.warnings.join('；')}</p>{t.changed_files.length > 0 && <p className="text-muted-foreground">已记录 {t.changed_files.length} 个变化文件</p>}</div>)}</details>}
      </div></SettingsCard>
      ...
      <SettingsCard divided={false}><details className="p-4 space-y-3"><summary className="text-sm font-semibold">⑥ 高级设置</summary>
        ...
        <div className="flex flex-wrap gap-2 text-sm">参数说明<SettingHelp topic="local-mcp-service" /><SettingHelp topic="remote-ingress-port" /><SettingHelp topic="connector-secret" /><SettingHelp title="Managed Bearer 与 Connector Secret">本机 Bearer 可选，公网始终通过 Connector Secret 授权。Secret 默认长期保存，新增项目不轮换。泄露时立即撤销远程访问；持有凭据者可调用已开启能力。</SettingHelp><SettingHelp title="远程 Scope">默认继承共享项目，也可选择子集。范围更改即时生效，不重启 Provider；同一个 Secret 下的项目属于同一信任域。</SettingHelp><SettingHelp title="Cloudflare Token 与 Tailscale Funnel">Cloudflare Named 使用 Tunnel Token 连接账号，和 MCP Secret 不同。Tailscale 需要登录、MagicDNS 与 Funnel 权限；本版拒绝覆盖已有 443 映射。</SettingHelp></div>
        ...
        <SettingField label="本机 MCP 服务端口（空表示自动）" helpTopic="local-mcp-service"><Input type="number" min={1024} max={65535} value={sharing.localEndpoint.port === 'auto' ? '' : sharing.localEndpoint.port} onChange={(e) => patchSharing({ localEndpoint: { ...sharing.localEndpoint, port: e.target.value ? Number(e.target.value) : 'auto' } })} /></SettingField>
        <label className="flex gap-2 text-xs"><input type="checkbox" checked={sharing.localEndpoint.auth === 'managed-bearer'} onChange={(e) => patchSharing({ localEndpoint: { ...sharing.localEndpoint, auth: e.target.checked ? 'managed-bearer' : 'none' } })} />本机 MCP 服务使用受管 Bearer 认证</label>
        <SettingField label="远程连接本机端口" helpTopic="remote-ingress-port"><Input type="number" min={1024} max={65535} value={remote.publicIngress.port} onChange={(e) => patchRemote({ publicIngress: { ...remote.publicIngress, port: Number(e.target.value) } })} /></SettingField>
        <div className="grid gap-3 text-xs sm:grid-cols-2"><p className="rounded bg-muted p-3">本机 MCP Client<br />↓<br />本机 MCP 服务 · 127.0.0.1:{sharing.localEndpoint.port}<br />↓ PROMA</p><p className="rounded bg-muted p-3">ChatGPT → 公网 HTTPS Domain<br />↓ ngrok / Cloudflare / Tailscale<br />远程连接本机端口 · 127.0.0.1:{remote.publicIngress.port}<br />↓ PROMA</p></div>
        ...
        <Button variant="outline" size="sm" onClick={() => { void navigator.clipboard.writeText(JSON.stringify({ provider: live?.kind, phase: live?.phase, endpoint: live?.endpoint, ngrok: live?.ngrok, binary: live?.binary, errorCode: live?.errorCode, probe: live?.probe, requests: live?.requests, logs: live?.logs, capabilities: { policy: sharing.policy, agentAction: sharing.delegation.action ?? { mode: 'analysis', write: false, execute: false } }, rootHealth: view.health, agentTargets: sharing.delegation.targets.map(({ id, channelId, modelId, enabled }) => ({ id, channelId, modelId, enabled })), attempts: view.tasks.map(({ id, status, attempts }) => ({ id, status, attempts })), discoveryFingerprint: live?.discoveryFingerprint, connectorUrlFingerprint: live?.connectorUrlFingerprint }, null, 2)).then(() => message('已复制安全诊断，不包含任务指令或内部 Agent 日志。')) }}>复制安全诊断</Button>
