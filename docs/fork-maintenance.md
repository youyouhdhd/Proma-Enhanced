...
### 2026-09-11：v1.12.1 本地运行时同步修复

旧 Bun 虚拟目录可能保留过期包；运行时复制入口现统一使用仓库固定的 hoisted 布局，避免旧缓存覆盖当前依赖。已新增默认来源选择回归测试，见 [v1.12.1 开发记录](../release-notes/v1.12.1.md)。此修复不移动 v1.11.0 标签，也不新增 GitHub Release。
...
- 合入内容详见 [v1.12.0 开发记录](../release-notes/v1.12.0.md)。当前正式版本仍为 v1.11.0，本次同步不移动已发布标签。
